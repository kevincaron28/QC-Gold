...
Production-oriented Discord guild management bot for Guilded, a World of Warcraft: Forever guild.

The bot is intentionally split into two boundaries:

- **Guilded** owns guild profiles, raids, signups, applications, permissions, announcements, and audit history.
- **Guilded** is the in-game EPGP/readiness authority. The addon exports data through a validated companion import adapter.
...
- A purpose-built `addon/Guilded` WoW addon baseline
...
- The addon's `/guilded inspect` now also captures known professions and skill levels; `/guilded attune` records attunement completion in-game (self or officer-for-other)
...
- The addon now auto-broadcasts a compact readiness/profession digest to the whole online guild (login, gear changes, every 10 min while raiding), which every other online client accumulates into its own `peerRoster` table — one officer's `/guilded export` can carry a readiness picture for the whole online guild, not just themselves. See `addon/Guilded/README.md` for details.
...
Not part of the roadmap, but added the same day: the WoW addon was restructured from a single `Guilded.lua` file into `Core.lua` + `Modules/`, and a games module was added. (The gambling minigames and their gold ledger that first lived there were removed in 2.4; `Modules/Games.lua` keeps only 1v1 and chat roll games.) See `addon/Guilded/Modules/README.md`.
...
- In game: a Dungeons tab (live run, recent runs and their sync state, season top 10 from Discord); `/testraid dungeon` and `/guilded sim dungeon` for testing, removed by `/testraid cleanup`

Version 3.0.1 (public release): see CHANGELOG.md. To publish the addon on CurseForge see docs/RELEASE_CURSEFORGE.md; the day-of checklist is TODAY_TODO.md. The companion has a desktop app (`companion-app/`).

Version 2.1 (addon and bot): per-core point rules (guild defaults, per-core overrides, optional separate point pools, per-core loot mode), one saved-data set per WoW guild in the addon, and `/guilded backup` / `/guilded restore`.

Version 2.0.0 (addon and bot):

- **Addon:** enchant check (which slots lack an enchant; reason flags travel in the guild digest), consumable scan, `/guilded share` (one paste to Discord's `/character sync`, no companion needed), login digest, `/guilded peers`, attendance snapshots, and a read-only `GuildedAPI` for other addons. The addon's real Lua files now run in the test suite against a mocked game (`tests/lua/`, fengari).
...
- `/guilded character` in game shows one line (name, realm, class, race, level, spec, professions); `/character import code:<line>` links or refreshes the character from it, no typing. The addon export also carries your own character block, which `/import-apply` uses to keep an already-linked character current
...
Addon modules (roadmap M1–M3): the addon stays **one** download. Roll games, GP bidding, Dungeons, Calendar and the test tools can be switched off per player (`/guilded modules off games`) or for the whole guild by an officer (`/guilded modules guild off games`, shared in game); raids, EPGP, loot, the gear check and standings always run. See `addon/Guilded/README.md`. Splitting it into separate CurseForge addons was considered and rejected: every part depends on the same core and talks to the others, so separate downloads would add version mismatches without giving guilds anything the switches don't.
...
(A public CurseForge listing is planned as roadmap item M3: one project, packaged from `addon/Guilded/` without `validate-addon.mjs`. Until then, releases go through GitHub as below.)
...
check your artifacts gallery for "Guilded Add-on", or ask to have it
...
1. Bump `## Version:` in `addon/Guilded/Guilded.toc` if this is a
   version worth shipping to guildies (not every internal commit needs a
   new release).
2. Run `npm run addon:zip` — builds `dist/Guilded-v<version>.zip` from
...
4. Publish a new [GitHub release](https://github.com/kevincaron28/QC-Gold/releases/new)
   tagged `v<version>` with `dist/Guilded-v<version>.zip` attached.
5. Update the install page artifact: new version number, new download URL
   (`https://github.com/kevincaron28/QC-Gold/releases/download/v<version>/Guilded-v<version>.zip`),
