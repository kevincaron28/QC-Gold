...
| `/guilded ready` / `ready ask` / `ready post` | Who in your raid or party is ready: a summary and everyone who is not. `ask` asks every addon in the group to check itself again (a ready check); `post` (officers) tells the group who needs attention. The window's **Ready** page shows the same list |
