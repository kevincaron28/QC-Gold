# Releasing on CurseForge (v3.2.0)
...
2. `node addon/Guilded/validate-addon.mjs` passes and `npm run addon:zip` builds
   `dist/Guilded-v3.2.0.zip` (top folder inside the zip must be `Guilded`).
...
- Files, Upload File: `dist/Guilded-v3.2.0.zip`.
...
- Changelog: paste the "3.2.0" section of CHANGELOG.md.
